<?php
/**
 * Class gatewaywelight_Model_Recurring - Responsible for Recurring payments operations with welight
 *
 * @author    Ricardo Martins
 * @copyright 2020 Magenteiro
 */
class gatewaywelight_Model_Recurring extends gatewaywelight_Model_Abstract
{
    ...
    const HEADER_V3_JSON
        = array('Accept: application/vnd.welight.com.br.v3+json;charset=ISO-8859-1',
    ...
    /** @var gatewaywelight_Helper_Recurring $_helper */
    protected $_helper;

    public function __construct()
    {
        $this->_helper = Mage::helper('gatewaywelight/recurring');
    }


    /**
     * Retrieve current status of the subscription at welight
     *
    ...
    public function getPreApprovalDetails($preApprovalCode, $isSandbox)
    {
        ...
        $key = $isSandbox ? $helper->getwelightProSandboxKey() : $helper->getwelightProNonSandboxKey();
    ...
    /**
     * Gets latest status from welight and Updates profile
    ...
    public function updateProfile($profile)
    {
        ...
        if ($currentStatus && isset($currentStatus->status)) {
            ...
            if ($profile->getState() != $oldState) {
                Mage::helper('gatewaywelight/recurring')->writeLog(
    ...
    /**
     * Update profile entity with the associated status from welight
     * @param Mage_Sales_Model_Recurring_Profile $profile
     * @param string $welightStatus Status code from welight
     *
     * @return mixed
     */
    public function updateProfileStatus($profile, $welightStatus)
    {
        switch ((string)$welightStatus) {
            case 'ACTIVE':
...
//                ->setPreparedMessage($this->_createIpnComment(''))
    ...
    public function getOrderPayments($subscription)
    {
        ...
        foreach ($apiReturn->paymentOrders as $order) {
            ...
            //https://dev.welight.uol.com.br/docs/pagamento-recorrente-tabelas-de-status-e-erros
    ...
    public function changewelightStatus($profile, $newStatus)
    {
        $helper = Mage::helper('gatewaywelight');
    ...
    public function cancelwelightProfile($profile)
    {
        $helper = Mage::helper('gatewaywelight');
        $preApproval = $profile->getReferenceId();
