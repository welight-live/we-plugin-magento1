 ...
 */
class gatewaywelight_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getwelightScriptBlock()
    {
        ...
        if (Mage::app()->getLayout()->getArea() == 'adminhtml') {
            $directPaymentBlock = Mage::app()->getLayout()
                ->createBlock('gatewaywelight/form_directpayment')
                ->toHtml();
        }
        ...
        $scriptBlock->setText(
            sprintf(
                ...
                Mage::helper('gatewaywelight')->getJsUrl(),
    ...
    public function getModuleJsUrl($secure)
    {
        if (Mage::getStoreConfigFlag(self::XML_PATH_JSDELIVR_ENABLED)) {
            ...
            $moduleVersion = (string)Mage::getConfig()->getModuleConfig('gatewaywelight')->version;
            $url
    ...
    {
        $moduleConfig = Mage::getConfig()->getModuleConfig('gatewaywelightPro');

        if (version_compare($moduleConfig->version, '3.3', '<')) {
            return false;
        }

        $rHelper = Mage::helper('gatewaywelightpro/retry');
    ...
    {
        $psVersion = (string)Mage::getConfig()->getModuleConfig('gatewaywelight')->version;
        $psProVersion = 'notInstalled';
        $mageVersion = Mage::getVersion();

        if (Mage::getConfig()->getModuleConfig('gatewaywelightPro')) {
            $psProVersion = (string)Mage::getConfig()->getModuleConfig('gatewaywelightPro')->version;
        }

        $userAgent = array('modules' => array('gatewaywelight'    => $psVersion,
                                              'gatewaywelightPro' => $psProVersion),
                           'magento' => $mageVersion);
