<?php
/**
 * Class gatewaywelight_KioskController
 *
 * @author    Ricardo Martins <ricardo@magenteiro.com>
 */
class gatewaywelight_KioskController extends Mage_Core_Controller_Front_Action
{
    ...
    /**
     * Create standalone/kiosk order and redirects to welight
     */
    public function createOrderAction()
    {
        $kHelper = Mage::helper('gatewaywelight/kiosk');
        $pHelper = Mage::helper('gatewaywelight');
        if (!$kHelper->isActive()) {
        ...
        try{
            $checkout = Mage::getModel('gatewaywelight/abstract')->callApi($params, null, 'checkout');
        }catch (Exception $e) {
        ...
        Mage::getModel('gatewaywelight/kiosk')
            ->setData($temporaryOrder->toArray())
            ->save();


        $this->_redirectUrl('https://welight.uol.com.br/v2/checkout/payment.html?code=' . $checkoutCode);
    ...
    public function successAction()
    {
        ...
        $kiosk = Mage::getModel('gatewaywelight/kiosk')->loadByTemporaryReference($temporaryReference);
        if (!$kiosk->getId()) {
