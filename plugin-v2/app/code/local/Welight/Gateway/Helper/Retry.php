...
/**
 * Class Welight_Gateway_Helper_Retry
 * Helper for Retry Process
 *
 * @author    Ricardo Martins <pagseguro-transparente@ricardomartins.net.br>
 */
class Welight_Gateway_Helper_Retry extends Mage_Core_Helper_Abstract
{
    ...
    {
        $isEnabled =  Mage::getStoreConfigFlag(Welight_Gateway_Model_Retry::XML_PATH_IS_ACTIVE);
        if (!$isEnabled) {
            return false;
        }
        $moduleConfig = Mage::getConfig()->getModuleConfig('Welight_Gateway');
    ...
    {
        return Mage::getStoreConfig(Welight_Gateway_Model_Retry::XML_PATH . $configNode);
    }
    ...
    {
        return Mage::getStoreConfigFlag(Welight_Gateway_Model_Retry::XML_PATH . $configNode);
    }
    ...
    public function sendRetryEmail($orderCancellation)
    {
        ...
        $mailer->setSender(Mage::getStoreConfig(Welight_Gateway_Model_Retry::XML_PATH_EMAIL_IDENTITY));
        $mailer->setStoreId($order->getStoreId());
        $mailer->setTemplateId(Mage::getStoreConfig(Welight_Gateway_Model_Retry::XML_PATH_EMAIL_TEMPLATE));
    ...
    {
        $code = Mage::getModel('welight_gateway/retry')->sendRetryPaymentRequest($order);
