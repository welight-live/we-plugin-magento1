<?php
class Welight_Gateway_Block_Adminhtml_Recurring_Sales_View extends Mage_Sales_Block_Recurring_Profile_View
{
    /**
    ...
    public function prepareReferenceInfo()
    {
        ...
        if ($tracker) {
            $link = "<a href=\"https://welight.uol.com.br/pagamento-recorrente/lista.html"
                . "?asBuyer=false&tracker=$tracker\" "
                . "target='_blank' title='ver no welight'>$tracker</a>";

            if ($this->_profile->getAdditionalInfo('isSandbox')) {
                $link = "<a href=\"https://sandbox.welight.uol.com.br/aplicacao/assinaturas/detalhes.html?code="
        ...
        $otherFields = array('Status welight'   => 'status', 'Reference' => 'reference',
                             'Última atualização' => 'last_event_date', 'Sandbox' => 'is_sandbox', 'Código do plano' => 'welightPlanCode');
    ...
    public function addToParentGroup($groupName)
    {
        ...
        if ($this->_profile && $this->_profile->getMethodCode() == 'rm_welight_recurring') {
